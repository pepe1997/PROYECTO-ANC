...
function login(event) {
  ...
  localStorage.setItem("tareas_asignacion_usuario", JSON.stringify({ user: valido.user, nombre: valido.nombre }));
  mostrarApp(valido);
  iniciarAplicacion();
}
...
function logout() {
  ...
  localStorage.removeItem("tareas_asignacion_usuario");
  mostrarLogin();
}
...
function cargarSesion() {
  ...
  const raw = localStorage.getItem("tareas_asignacion_usuario");
